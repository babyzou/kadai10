...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
	...
	if ($form['email'] === '') {
        ...
        $db = dbconnect();
        $stmt = $db->prepare('select count(*) from members where email=?');//件数をカウントするSQL
        if(!$stmt){
            die($db->error);
        }
    ...
    //画像のチェック
    $image = $_FILES['image'];
    if ($image['name'] !== '' && $image['error'] === 0){//ファイルに名前が入っていてかつ、エラーが起こっていないかチェック
        $type = mime_content_type($image['tmp_name']);
        if ($type !== 'image/png' && $type !== 'image/jpeg'){//pngでもjpegでもない場合
            $error['image'] = 'type';
        }
    }
			...
			<form action="" method="post" enctype="multipart/form-data">
				<dl>
					...
					<dd>
						...
						<?php if (isset($error['password']) && $error['password'] === 'bldddddddddank') : ?>
